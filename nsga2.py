...
class NSGA2_Distribute():
        ...
        for r in range(self.POP_SIZE):
            ...
            for k in range(amf_number):
                ...
                for m in avail_index:
                    # distance.append(nx.dijkstra_path_length(sat960_main.G, self.P0[r][k], m))
                    distance.append(sat960_main.delay[self.P0[r][k]][m])
            ...
            while i<smf_number:
                ...
                for  a in range(amf_number):
                    # amf_smf_dis[a][i] = nx.dijkstra_path_length(sat960_main.G, self.P0[r][a], self.P0[r][self.MAX_AMF_NUMBER + i])
                    amf_smf_dis[a][i] = sat960_main.delay[self.P0[r][a]][self.P0[r][self.MAX_AMF_NUMBER + i]]
            ...
            for k in range(smf_number):
                distance = []
                for m in avail_index:
                    # distance.append(nx.dijkstra_path_length(sat960_main.G, self.P0[r][self.MAX_AMF_NUMBER + k], m))
                    distance.append(sat960_main.delay[self.P0[r][self.MAX_AMF_NUMBER + k]][m])
            ...
            while i < upf_number:
                ...
                for s in range(smf_number):
                    # smf_upf_dis[s][i] = nx.dijkstra_path_length(sat960_main.G, self.P0[r][self.MAX_AMF_NUMBER + s], self.P0[r][self.MAX_AMF_NUMBER + self.MAX_SMF_NUMBER + i])
                    smf_upf_dis[s][i] = sat960_main.delay[self.P0[r][self.MAX_AMF_NUMBER + s]][self.P0[r][self.MAX_AMF_NUMBER + self.MAX_SMF_NUMBER + i]]
    ...
    def get_fitness(self, index):
        ...
        capital, performance, result,_,_,_,_ = sat960_main.F(len(l_amf), len(l_smf), len(l_upf), amf, smf, upf)
